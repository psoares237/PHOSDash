...
BG_WATERMARK = os.path.join(BASE_DIR, "assets", "bg_watermark.png")

# ── Watermark background ──
if os.path.exists(BG_WATERMARK):
    with open(BG_WATERMARK, "rb") as _f:
...
def standardize(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # MesLabel para cross-filtering — mesma formatação do utils.formatters.month_label
    if "MesLabel" not in df.columns and "Mes" in df.columns:
...
# ── Header com logo miniatura ──
if os.path.exists(LOGO_SIDEBAR):
    with open(LOGO_SIDEBAR, "rb") as _lf:
...
_nome_pagina = pagina.replace("📊 ", "").replace("🎯 ", "").replace("💰 ", "")
st.markdown(
    f"""
<div class="top-header">
{_logo_html}
<div class="top-header-info">
<div class="top-header-brand">PHOSDash</div>
<div class="top-header-page">{_nome_pagina}</div>
</div>
</div>
""",
    unsafe_allow_html=True,
...
if pagina == "📊 Visão Geral Operacional":
    ...
    from views.Financeiro import render as render_financeiro

    render_financeiro(ctx)